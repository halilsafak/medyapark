...
function render(){ if(view.type==='home')renderHome(); else if(view.type==='mec')renderMec(); else if(view.type==='alt')renderAlt(); else renderPage(view.slug); window.scrollTo({top:0,behavior:'smooth'}); }
...
function openAlt(mecId,altId){ view={type:'alt',mecId,altId,gsub:'galeri',gidx:0}; calYear=new Date().getFullYear(); render(); }
function openAltTab(altId){ view.altId=altId; view.gidx=0; renderAlt(); }
function gSub(s){ view.gsub=s; renderAlt(); }
...
/* ---------- ANASAYFA: sadece MECRALAR ---------- */
function renderHome(q){
  q=(q||'').trim().toLowerCase();
  const list=D.mecralar.filter(m=>!q||m.name.toLowerCase().includes(q)||(m.alts||[]).some(a=>a.name.toLowerCase().includes(q)));
...
function renderMec(){
  ...
  const cards=(m.alts||[]).map(a=>{ const p=a.product||{};
    const sub=[a.gunluk_gosterim||p.olcu, a.toplam_alan].filter(Boolean).join(' · ');
    return cardTile(`openAlt('${m.id}','${a.id}')`, a.name, sub, a.image||m.image, m.theme_color, 'Detaya gir');}).join('');
  app().innerHTML=`<div class="crumbs"><a href="#" onclick="goHome();return false;">Katalog</a> › <span>${esc(m.name)}</span></div>
    ...
    <div class="grid">${cards||'<p class="muted">Bu mecrada alt mecra yok.</p>'}</div>`;
}
...
function renderAlt(){
  ...
  const alts=m.alts||[];
  let alt=alts.find(a=>String(a.id)===String(view.altId))||alts[0];
  if(!alt)return renderMec();
  const p=alt.product||{}; const uList=alt.units||[];

  const tabs=alts.map(a=>`<button class="mtab ${String(a.id)===String(alt.id)?'on':''}" onclick="openAltTab('${a.id}')">${esc(a.name)}</button>`).join('');

  const gsub=view.gsub||'galeri'; const gal=Array.isArray(alt.galeri)?alt.galeri:[];
  const gidx=Math.min(view.gidx||0, Math.max(0,gal.length-1));
  let media;
  if(gsub==='yerlesim'){ media = alt.yerlesim_plani?`<div class="gmain" style="background-image:url('${esc(alt.yerlesim_plani)}')"></div>`:`<div class="gmain">Yerleşim planı yok</div>`; }
  ...
  const mapsBlock = vis(alt,'maps',!!alt.maps) ? `<div class="mapbox">${alt.maps}</div>` : `<div class="mapbox">Google Maps</div>`;
  const adv=(Array.isArray(alt.avantajlar)?alt.avantajlar:[]).slice(0,4);
  const advBlock = vis(alt,'avantajlar',adv.length>0) ? `<div class="adv-grid">${adv.map(a=>`<div class="adv"><b>${esc(a.t||a.title||'')}</b><span>${esc(a.d||a.desc||'')}</span></div>`).join('')}</div>` : '';
  ...
  const specRows=[['Ürün',p.name],['Ölçü',p.olcu],['Yüzey',p.yuzey],['Aydınlatma',p.isikli],['Baskı Malzemesi',p.baski_malzemesi],['Baskı Formatı',p.baski_format],['Yayın Formatı',p.yayin_format],['Baskı Ücreti',p.baski_ucreti],['Montaj Ücreti',p.montaj_ucreti]]
  ...
  app().innerHTML=`
    <div class="crumbs"><a href="#" onclick="goHome();return false;">Katalog</a> › <a href="#" onclick="openMec('${m.id}');return false;">${esc(m.name)}</a> › <span>${esc(alt.name)}</span></div>
    <div class="sechead"><h1>${esc(alt.baslik||alt.name)}</h1>${alt.aciklama?`<p>${esc(alt.aciklama)}</p>`:''}</div>
    ${alts.length>1?`<div class="mtabs">${tabs}</div>`:''}
    <div class="u-grid">
    ...
    <div class="book" style="margin-top:18px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        <h4 style="margin:0">${esc(alt.name)} — Doluluk</h4>
      ...
      <p class="muted" style="font-size:13px;margin:0 0 12px">Müsait bir aya tıklayarak teklif sepetine ekleyin (aylık: <b>${money(perMonth(p))}</b>).</p>
      <div style="overflow-x:auto">${matrixHTML(p.name||'Pozisyon', uList)}</div>
...
function yearNav(d){ calYear+=d; renderAlt(); }
function toggleMonth(uid,ym){
  ...
  if(i>-1){ cart.splice(i,1); }
    ...
    cart.push({unitId:Number(uid),mecra:m.name,alt:alt.name,unit:u.name,product:p.name,olcu:u.olcu||p.olcu||'',ym,
...
function renderCart(){
  ...
  b.innerHTML=cart.map((c,i)=>`<div class="citem"><button class="rm" onclick="removeItem(${i})">×</button>
    <div class="cl">${esc(c.unit)}</div><div class="cm">${esc(c.mecra)}${c.alt?' · '+esc(c.alt):''} · ${esc(c.product)}</div>
...
function exportExcel(){
  if(!cart.length)return;
  const head=['Lokasyon','Alt Mecra','Ünite','Ürün','Ölçü','Dönem','Fiyat'];
  const rows=cart.map(c=>[c.mecra,c.alt||'',c.unit,c.product,c.olcu,c.monthLabel,c.priceLabel]);
...
function exportPDF(){
  ...
  const rows=cart.map(c=>`<tr><td>${esc(c.mecra)}${c.alt?' / '+esc(c.alt):''}</td><td>${esc(c.unit)}</td><td>${esc(c.product)}</td><td>${esc(c.olcu)}</td><td>${esc(c.monthLabel)}</td><td style="text-align:right">${c.priceLabel}</td></tr>`).join('');
  const w=window.open('','_blank');
  w.document.write(`<html><head><meta charset="utf-8"><title>Teklif</title><style>body{font-family:-apple-system,Inter,Arial;padding:40px;color:#1d1d1f}h1{font-size:22px}table{width:100%;border-collapse:collapse;font-size:13px}th,td{padding:9px;border-bottom:1px solid #eee;text-align:left}th{color:#6e6e73;font-size:11px;text-transform:uppercase}tfoot td{font-weight:700;border-top:2px solid #1d1d1f}</style></head><body>
    <h1>${esc(s.logoText||'Medyapark')} — Teklif Listesi</h1><p style="color:#6e6e73">${new Date().toLocaleDateString('tr-TR')} · ${esc(s.phone||'')}</p>
